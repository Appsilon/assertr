...
### success, error and skip functions

The behavior of functions like `assert`, `assert_rows`,
`insist`, `insist_rows`, `verify` when the assertion
passes and fails is configurable via the `success_fun`, `error_fun` and `skip_fun` parameters.
...
provided by this package:
  ...
  * `append_as_warning` - Attaches the error as warning rule to a special
   attribute of `data` and returns the data.
...
The `skip_fun` parameter takes a function that takes
the data passed to the assertion function as a parameter. When some of 
obligated data assertion fails, we are not able to perform following checks.
Corrupted data is not validated and the function set an action about what should 
be done during current assertion.
You can use here all of above methods. The default one is `success_continue` what
means that thw whole data is passed to another rule.

See below example when one of requires column is missing:
```{r purl=FALSE, error=TRUE}
mistake <- iris
mistake$Species <- NULL

mistake %>% 
  verify("Species" %in% names(.)) %>% 
  verify(in_set("setosa", "versicolor", "virginica"), Species)

mistake %>% 
  verify("Species" %in% names(.), mark_data_corrupted_on_failure = TRUE) %>% 
  verify(in_set("setosa", "versicolor", "virginica"), Species)
```
...
You may want to store validation results within specified object.
The best way for it is to create new validator object:
```{r purl = FALSE}
validator <- Validator$new()
```

To write validation result in it just use:
```{r purl = FALSE}
mtcars %>%
  chain_start %>% 
  verify(title = "Data should have more that 10 rows.", nrow(.) > 10) %>%
  verify(title = "mpg is positive.", mpg > 0) %>%
  assert(title = "am and vs should have just 0 and 2 values.", in_set(0,2), am, vs) %>% 
  chain_end(error_fun = error_append) %>% 
  validator$add_validations("mtcars")
```

And check the results in required format by:
```{r purl = FALSE}
